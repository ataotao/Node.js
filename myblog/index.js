/**
 * 注意：中间件的加载顺序很重要。如下面设置静态文件目录的中间件应该放到 routes(app) 之前加载，这样静态文件的请求就不会落到业务逻辑的路由里；flash 中间件应该放到 session 中间件之后加载，因为 flash 是基于 session 的。
 */
...
// 处理表单及文件上传的中间件 使用 express-formidable 处理 form 表单（包括文件上传）。
...
/**
 * app.locals 和 res.locals 使用上的区别在于：
...
/**
 * 我们将正常请求的日志打印到终端并写入了 logs/success.log，将错误请求的日志打印到终端并写入了 logs/error.log。
